...
`styles.css` defines the shared light and dark themes, applies the portable reset, and includes both legacy CSS and extracted StyleX component recipes. It also registers the package source with Tailwind while the compatibility bridge exists. Consumers do not need a fragile `node_modules`-relative `@source` path. The stylesheet expects Tailwind CSS v4 processing during this transition; it deliberately does not import Tailwind itself, which prevents duplicate Preflight and utility output.
...
`tokens.css` and `reset.css` are standards-only CSS and do not require Tailwind. `stylex.css` is generated by the package build and contains the migrated component recipes in the `components.hraness-ui` layer namespace. `tailwind.css` owns only source detection, the dark variant, and semantic Tailwind mappings.
