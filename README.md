...
{
  "dependencies": {
    "@hraness/ui": "github:hraness/ui#v0.5.14"
  }
}
```
...
The Next adapter has exact production profiles for Next 16.2.12 (the unchanged default) and explicitly selected 16.3.3, with webpack 5 under Node 24. Set `nextVersion: "16.3.3"` in both `withStylexNext` and `runStylexNextBuild` to select the latter; the installed package must match exactly. There is no automatic installed-version selection or semver-range admission. Each profile binds its own framework, proxy/SSG, empty-entry and native type-writer hashes into the same complete plan/graph/type/postprocessing joins. The 16.3.3 profile additionally observes `root-params.d.ts` and its native `next-env.d.ts` import; a 16.2.12 inventory cannot claim that new file as its native pre-webpack output. Every historical inventory remains bound to its own exact profile, including when a later build selects the other supported version.

`test:next-adopter` runs isolated packed consumers for both exact versions. The 16.3.3 row also calls its hash-verified installed root-parameter writer under Node 24 for all seven nonempty return-type unions, both name orders and the empty declaration. That separate writer probe is not full dynamic-root route acceptance: the complete compiler fixture has a static root layout and proves its native empty root-parameter declaration, both pre-webpack writer observations and the complete discovery/delivery joins.

Both pinned frameworks write a different client-reference manifest path than they read for a static App Router identity beginning with `/index`, so the adapter rejects that unsupported route before accepting the manifest. Dynamic descendants such as `/index/[slug]` remain supported and may statically generate a concrete URL such as `/index/example`. The adapter runs two ordinary `next build --webpack` processes because Next owns separate client, Node React Server Component, and edge React Server Component compilers and exposes no supported cross-compiler completion barrier in `next.config`. The discovery build records every repository JavaScript and TypeScript input, chained source map, raw StyleX rule, registered package manifest, CSS input, and emitted asset. The delivery build must reproduce the same source and transform receipts before it can add the single serialized rule union through a native CSS import. Missing, duplicate, or unexpected targets and inter-pass drift stop the build.
...
Both exact profiles also admit Next's built-in global-error entry when an application has no physical `app/global-error`. That framework fallback does not own the application stylesheet. Its exact component and entry creators are checked at graph publication and settlement; nested application boundaries and lookalike paths remain unsupported. The packed test retains the physical error-boundary browser proof and adds a separate discovery/delivery build with the default boundary.
...
Both exact profiles rename the registered proxy's `server/proxy.js` and dependency trace to `server/middleware.js` and its trace after webpack, without renaming `server/proxy.js.map`. A proxy-only `proxyRename` branch in the postprocessing snapshot binds that version's exact native writer, unchanged JavaScript and map bytes, and absence of both original paths. The original graph keeps its compiler-owned names; no other runtime asset is renamed or exempted. Older snapshot readers do not accept this new branch. The final trace file list remains an observation, including any native self-reference rewrite.
