...
- `next-profile.ts` binds each exact supported production version to its immutable framework, postprocessing and native type-writer identities; 16.2.12 remains the default.
...
- Keep Next production profiles exact: default 16.2.12 or explicitly selected 16.3.3, with webpack and genuine Node 24. Never infer a profile from installed dependencies or widen a version range. Bind plans, graphs, postprocessing and type inventories to the same selected version and its complete creator-hash matrix. Run the same packed native qualification for both versions, with complete discovery and delivery builds for the client, Node RSC, and Edge RSC targets. Bind each target to the exact repository-source census in the hash-bound plan; an empty target is valid only when the compiler observes no repository source and still emits both target receipts.
