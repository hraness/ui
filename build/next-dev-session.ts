...
function renderSnapshotCss(
  ...
  const union = serializeStylexRuleUnionV1(rules, manifests.map(({ standaloneSerializer }) => standaloneSerializer));
  const prelude = /^(?:@layer [^;{}]+;\n)+/u.exec(union);
  ...
  const prepare = async (): Promise<NextDevPreparation> => {
    ...
    try {
      ...
      for (const foundation of foundations) {
        auditCssWithoutStandaloneRecipes(await readFile(resolve(dirname(cssEntry), foundation), "utf8"), manifests, "Next development foundation");
      }
      const sourceIdentity = inputs.sources.map(({ logicalPath, sourceSha256 }) => ({ logicalPath, sourceSha256 }));
      const revision = sha256(canonicalJson({ compilerSha256, options: { ...options, rootDirectory: "<root>" }, packages: manifests.map((manifest) => sha256(canonicalJson(manifest))), sources: sourceIdentity, unionPolicySha256: stylexUnionPolicySha256 }));
      ...
      const css = renderSnapshotCss(revision, includedRevisions, [], rules, foundations, manifests);
