...
function snapshotBundle(
  ...
  if (mapPaths !== undefined) {
    for (const chunk of Object.values(bundle)) {
      ...
      assert.equal(typeof chunk.preliminaryFileName, "string", "Vite source maps require the native preliminary chunk identity");
      ...
      const options: Parameters<typeof validateViteSourceMap>[1] = { chunkPath: chunk.fileName,
        preliminaryChunkPath: chunk.preliminaryFileName!, requiredSources, code: chunk.code, paths: mapPaths };
  ...
  return {
    ...
    generateBundle: {
      ...
      async handler(outputOptions, bundle) {
        ...
        const bundleSnapshot = snapshotBundle(bundle, rootDirectory, mapPaths);
    ...
    writeBundle: {
      ...
      async handler(outputOptions, bundle) {
        ...
        assert.deepEqual(snapshotBundle(bundle, rootDirectory, mapPaths), generatedBundle,
