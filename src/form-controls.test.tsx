...
test("form, select, and checkbox composition keeps native submission semantics", () => {
  ...
  expect(html).toMatch(/class="hraness-select-field(?: [^"]+)*"/u);
  expect(html).toContain('data-slot="select-field"');
  expect(html).toMatch(
    /class="hraness-select-field__indicator(?: [^"]+)*"/u,
  );
  expect(html).toContain('data-slot="select-field-indicator"');
