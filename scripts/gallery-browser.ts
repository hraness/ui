...
try {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        invariant(
          counterfactual.keyHintLayerSentinels
          && !counterfactual.keyHintBoundaryContracts
          && counterfactual.keyHintDefaultDisplay === "block",
          `priority3-before-legacy counterfactual: KeyHint did not flip to the legacy recipe: ${counterfactual.keyHintDiagnostics}`,
        );
        invariant(
  ...
  console.log(
    "Primitive gallery browser passed: packed default CSS and priority3 layer order, matched gallery-only conflicts losing to StyleX in production, a served priority3-before-legacy counterfactual flipping footer padding and KeyHint presentation to legacy values, SSR/hydration, semantic StyleX glyph, wrapper, quiet-site landmarks, horizontal and vertical structural-surface layout behavior, viewport height fallbacks, every themed-surface tone and shape, caller-last texture composition, Avatar fallback sizes, data-URI image cropping, Badge, Tag, StatusDot, KeyHint, Card, PressableCard, and Toolbar finite recipes, public Tag accent, public Card description overrides and nested tone resets, caller and native interaction precedence, Toolbar native and caller keyboard focus, compact/short layouts, light/dark, reduced motion, forced colors, network/console diagnostics, and cleanup.",
  );
} finally {
