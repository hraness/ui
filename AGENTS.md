...
- `src/navigation.tsx`, `src/quiet-site.tsx`, `src/quiet-site.stylex.ts`, `src/router.tsx`, `src/skip-link.tsx`, `src/surfaces.tsx`, and `src/toolbar.tsx` – navigation, package-compiled quiet-site landmarks, framework-neutral router integration, and structural layout primitives.
...
- Under StyleX 0.19 property-specificity resolution, author writing-mode-sensitive `inline-size`, `max-inline-size`, and `min-inline-size` with canonical dashed keys. Quiet-site `xstyle` types must reject the camel-case aliases because they lower to physical width declarations and do not share the same caller-last conflict key; physical callers use `width`, `maxWidth`, and `minWidth`. Keep artifact, external package-type, and vertical-browser evidence for this boundary.
