...
- `src/tokens.css`, `src/reset.css`, `src/components.css`, `src/tailwind.css`, and `src/styles.css` – portable tokens, baseline, remaining component recipes, the transition Tailwind bridge, and the ordered public stylesheet.
...
- Keep `dist/stylex.css` inside the `components.hraness-ui` layer namespace. The complete stylesheet imports it after legacy component recipes and before the transition Tailwind bridge so existing utilities keep their documented override priority.
