...
- `src/avatar.stylex.ts`, `src/badge.tsx`, `src/card.tsx`, `src/content.tsx`, `src/data-display.tsx`, `src/feedback.tsx`, `src/icon.tsx`, `src/icon.stylex.ts`, `src/indicators.tsx`, and `src/status.stylex.ts` – status, surface, content, data, feedback, and shared icon and avatar primitives, including package-compiled StyleX recipes.
