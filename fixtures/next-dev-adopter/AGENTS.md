...
- `../../scripts/next-dev-adopter-smoke.ts` runs the packed matrix in an owned Node 24 worker. The Bun coordinator independently verifies its terminal process-group, stream, browser, and loopback-listener collection before publishing durable evidence.
...
- Keep Playwright's server, connection, contexts, and assertions in the same genuine Node 24 worker. Register pending acquisitions before awaiting them. Collect every reported process group and browser child before a terminal record; cancellation or uncertain collection retains evidence and forbids success. The coordinator must verify the request-bound worker record after the worker exits and publish the matrix outside disposable consumers.
