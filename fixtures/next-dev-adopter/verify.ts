...
export async function verifyNextDevAdopter(page: Page, origin: string, fixture: string, hasEdge: boolean): Promise<Readonly<{ coherenceFrames: number; sealedDocuments: readonly { route: string; samples: number }[]; stages: readonly string[] }>> {
  ...
  try {
    ...
    await eventually(async () => {
      const response = await page.request.get(`${origin}/`, { maxRedirects: 0 });
    ...
    await eventually(async () => (await page.request.get(`${origin}/`, { maxRedirects: 0 })).status() === 200, "Next did not recover after the source repair");
    await preserveState();
    expectedCompilationError = null;
    stages.push("failed-revision-recovery");
    expectedCompilationError = /Next development relative import cannot be resolved inside its root: app\/recovery\.stylex\.ts -> \.\/created-later/u;
    await writeFile(join(fixture, "app/recovery.stylex.ts"), 'export { recovery } from "./created-later";\n');
    await eventually(async () => {
      const response = await page.request.get(`${origin}/`, { maxRedirects: 0 });
      return response.status() === 500 && (await response.text()).includes("created-later");
    }, "Missing relative import did not report its exact failed resolution");
    assert.ok((await css()).includes("117.125px"), "Missing input discarded last-good CSS");
    // Only the missing candidate is created. No touch of the importer, restart,
    // or unrelated edit may rescue a broken missingDependencies watcher.
    await writeFile(join(fixture, "app/created-later.tsx"),
      'import * as stylex from "@stylexjs/stylex"; export const recovery = stylex.create({ unused: { marginLeft: 119.125 } });\n', { flag: "wx" });
    missingOwned = true;
    await eventually(async () => (await page.request.get(`${origin}/`, { maxRedirects: 0 })).status() === 200
      && (await css()).includes("119.125px") && !(await css()).includes("117.125px"),
    "Creating the exact missing candidate did not recover native compilation and replace stale CSS");
    await preserveState();
    expectedCompilationError = null;
    stages.push("missing-resolution-candidate-recovery");
    await writeFile(join(fixture, "app/recovery.stylex.ts"), originals.get("app/recovery.stylex.ts")!);
    ...
    try {
      ...
      if (missingOwned) await unlink(join(fixture, "app/created-later.tsx"));
    }
  }
}
  ...
  try {
    for (const [before, after, stage] of [
      ...
      while (Date.now() < deadline) {
        const response = await context.request.get(`${origin}/`, { maxRedirects: 0 });
